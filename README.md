...
- **真动画素材**（已立项，待专门处理）：用图生视频生成各状态动作短片（走路步态 / 趴睡呼吸 / 作揖提醒），通过视频抽帧管线接入——管线与 prompt 已就绪，见 [docs/sprite-spec.md](docs/sprite-spec.md) 的「视频工作流」章节；需要 `brew install ffmpeg`
- 走路摇摆动效参数继续调优（配合多帧素材）
- 打包发布 v2.1.0 并打 tag 触发 CI 三平台构建
